function [Y_min,Y_max,X_min,X_max,out_min,out_max] = interval_bound_propigation(u_min,u_max,dim_hidden,net)
...
for k = 2:num_layers+1 %2:num_layers+1
...
%     
%     if strcmp(AF, 'relu')
%         x_min{k} = max(0,z_min{k});
%         x_max{k} = max(0,z_max{k});
%     elseif strcmp(AF, 'sigmoid')
%         x_min{k} = 1./(1+exp(-z_min{k}));
%         x_max{k} = 1./(1+exp(-z_max{k}));
%     elseif strcmp(AF, 'tanh')
%         x_min{k} = tanh(z_min{k});
%         x_max{k} = tanh(z_max{k});
%     end
%         
% end
