...
repeated = 0;
tic 
[eq_constraints, ineq_constraints] = hiddenLayerConstraints2(net,AF,alpha,beta,u_min,u_max,u,x,y,dim_in,dim_hidden,dim_out,repeated);
...
% 
% % Function for hidden layer constraints
% [eq_constraints, ineq_constraints] = hiddenLayerConstraints(alpha,beta,Ip,In,Ipn,u,x,y);
% 
...
%             ineq_constraints{icount} = x_curr_layer(k) - 0.05*v{j}(k); icount = icount + 1;
%             ineq_constraints{icount} = x_curr_layer(k) - 0.95*v{j}(k); icount = icount + 1;
