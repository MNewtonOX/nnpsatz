...
eps = 0.0001;
v1 = W{1}*u + b{1};
con_hid11a = (x11 - eps)*(x11 - v1(1) + eps);
con_hid11b = (eps - x11)*(eps - x11 + v1(1));
con_hid11c = x11*(x11 - v1(1));

con_hid12a = (x12 - eps)*(x12 - v1(2) + eps);
con_hid12b = (eps - x12)*(eps - x12 + v1(2));
con_hid12c = x12*(x12 - v1(2));

v2 = W{2}*[x11;x12] + b{2};
con_hid21a = (x21 - eps)*(x11 - v2(1) + eps);
con_hid21b = (eps - x21)*(eps - x21 + v2(1));
con_hid21c = x21*(x21 - v2(1));

con_hid22a = (x22 - eps)*(x22 - v2(2) + eps);
con_hid22b = (eps - x22)*(eps - x22 + v2(2));
...
for j = 1:10
    [prog,s{j}] = sospolyvar(prog,monomials(vars,0:0));
    [prog] = sosineq(prog,s{j});
end

% Polynomials for ideal
for j = 1:4
    [prog,t{j}] = sospolyvar(prog,monomials(vars,0:0));
end
